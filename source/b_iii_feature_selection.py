...
def sequential_forward_selection(data_features, labels, num_tests, feature_counts):
    '''
    ...
    for nfeat in feature_counts:
        ...
        # train
        sfs = sfs.fit(data_features, labels)
...
def fischer_criterion(data, labels, feature):
    '''
    inputs: DataFrame including features and labels ('covid(label)'),
                    feature 'd',
    ...
    classes = labels.unique() # number of classes
    numerator = 0
    denomenator = 0
    ud = data[feature].mean() # average of all values in feature / mu d
    for each_class in classes: # for each class / sum k
        vals_in_class = data.loc[labels == each_class] # create subdf with all values in given class
        ukd = vals_in_class[feature].mean() # average of feature values in given class / mu kd
        for xnd in vals_in_class[feature]: # for each feature value in class / sum xn
    ...
    # avoid empty
    if denomenator == 0:
        final_fischers_criterion = float('inf')
    else:
        final_fischers_criterion = numerator / denomenator

    return final_fischers_criterion

    

def fischer_criterion_selection(data, labels, feature_counts):
    '''
...
if __name__ == "__main__":
    ...
    # loading features df
    hog_feature_vectors = create_feature_df(hog_feature, image_size=100) # 1157 features...
